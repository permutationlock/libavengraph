    ...
    static inline void graph_plane_p3color_geometry_push_ctx(
        ...
        for (size_t i = 0; i < active_frames.len; i += 1) {
            GraphPlaneP3ColorFrameOptional *maybe_frame = &get(active_frames, i);
            if (maybe_frame->valid) {
                GraphPlaneP3ColorFrame *frame = &maybe_frame->value;

                GraphPlaneP3ColorVertex fu_info = get(
                    ctx->vertex_info,
                    frame->u
                );
        ...
        while (1) {
            if (active) {
                while (frame_index < active_frames.len) {
                    if (get(active_frames, frame_index).valid) {
                        maybe_cur_frame.value = &get(active_frames, frame_index)
                            .value;
            ...
            GraphPlaneP3ColorVertex cfu_info = get(
                ctx->vertex_info,
                cur_frame->u
            );
            ...
            for (
                ...
                if (v1 > v2) {
                    ...
                    GraphPlaneP3ColorGeometryDrawSlice v2_drawn = get(
                        draw_graph,
                        v2
                    );
                    ...
                    GraphPlaneP3ColorGeometryDrawSlice v1_drawn = get(
                        draw_graph,
                        v1
                    );
