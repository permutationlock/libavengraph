    ...
    static inline void graph_plane_p3choose_geometry_push_frame_active(
        ...
        aff2_add_vec2(node_active_trans, node_active_trans, z_pos);
        ...
        // Draw edges of outer cycle including active edge
        {
            ...
            do {
                ...
                if (v == frame->z) {
                    ...
                    aff2_add_vec2(node_x_trans, node_x_trans, v_pos);
                    aven_gl_shape_rounded_geometry_push_square(
                        rounded_geometry,
                        node_x_trans,
                        1.0f,
                    ...
                    aff2_add_vec2(node_y_trans, node_y_trans, v_pos);
                    aven_gl_shape_rounded_geometry_push_square(
                        rounded_geometry,
                        node_y_trans,
                        1.0f,
                    ...
                    aff2_add_vec2(node_face_trans, node_face_trans, v_pos);
                    aven_gl_shape_rounded_geometry_push_square(
                        rounded_geometry,
                        node_face_trans,
                        1.0f,
            ...
            GraphPlaneGeometryEdge edge_info = { .thickness = edge_thickness };
            ...
            do {
                ...
                if (v_loc->nb.first == v_loc->nb.last) {
                    uint32_t u = graph_aug_nb(ctx->nb, v_adj, v_loc->nb.last)
                        .vertex;
                    // if (u >= v) {
                    graph_plane_geometry_push_edge(
                        geometry,
    ...
    static inline void graph_plane_p3choose_geometry_push_ctx(
        ...
        // Draw all graph edges
        {
            ...
            for (uint32_t v = 0; v < ctx->vertex_info.len; v += 1) {
                GraphAdj v_adj = get(ctx->vertex_info, v).adj;
                GraphPlaneP3ChooseList *v_colors = &get(ctx->vertex_info, v)
                    .colors;
                for (uint32_t i = 0; i < v_adj.len; i += 1) {
                    uint32_t u = graph_aug_nb(ctx->nb, v_adj, i).vertex;
                    if (u < v) {
                    ...
                    GraphPlaneP3ChooseList *u_colors = &get(ctx->vertex_info, u)
                        .colors;
                    if (
                        v_colors->len == 1 and
                        u_colors->len == 1 and
