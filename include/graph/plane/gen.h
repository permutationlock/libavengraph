    ...
    static inline GraphPlaneGenData graph_plane_gen_grid(
        ...
        return (GraphPlaneGenData){ .graph = graph, .embedding = embedding };
    }
    ...
    static inline GraphPlaneGenTriangulationCtx
        graph_plane_gen_triangulation_init(
            GraphPlaneEmbedding embedding,
            Aff2 trans,
            float min_area,
            float min_coeff,
    ...
    static inline bool graph_plane_gen_triangulation_step(
        ...
        float coeff[3] = { min_coeff, min_coeff, min_coeff };
        ...
        for (uint32_t i = 0; i < valid_neighbors.len; i += 1) {
            ...
            for (; j < 3; j += 1) {
                if (
                    neighbor_prev_neighbor->vertices[j] == new_face->vertices[2]
                ) {
                    break;
                }
            }
            ...
            for (j = 0; j < 3; j += 1) {
                if (
                    new_face_next_neighbor->vertices[j] == new_face->vertices[0]
                ) {
                    break;
                }
            }
    ...
    static inline GraphPlaneGenData graph_plane_gen_triangulation(
        ...
        Graph graph = graph_plane_gen_triangulation_graph_alloc(size, arena);
